...
fn map_json(map: PatriciaMap<Vec<(usize, usize)>>) -> JSONMap<String, JSONValue> {
...
impl Index {
    ...
    fn into_minisearch_json(self) -> String {
        ...
        h.insert(
            "fieldIds".to_string(),
            field_ids_json(self.field_ids).into(),
        );
        h.insert(
        ...
        h.insert(
            ...
            field_length_json(self.field_length).into(),
        );
        h.insert("index".to_string(), map_json(self.map).into());
